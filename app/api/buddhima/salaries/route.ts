...
export async function GET() {
  ...
  try {
    ...
    const salaries = (await sql`
      SELECT id, member_id, project_id, amount, currency,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const [salary] = (await sql`
      INSERT INTO salaries (member_id, project_id, amount, currency, paid_on, note)
      VALUES (
        ${memberId}, ${projectId}, ${amount}, ${currency},
        COALESCE(${paidOn}::date, CURRENT_DATE), ${note}
      )
